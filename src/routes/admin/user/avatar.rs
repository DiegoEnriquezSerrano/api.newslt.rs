...
#[put("/user/avatar")]
#[tracing::instrument(
  name = "Updating user profile avatar",
  skip_all,
  fields(user_id=%*user_id)
)]
pub async fn put(
    cloudinary_client: web::Data<CloudinaryClient>,
    params: web::Json<UpdateAvatarParams>,
    ...
    let uploaded_image = cloudinary_client
        .upload_avatar(image.as_ref().to_string(), &user_id)
        .await?;
    let content = cloudinary_client.get_image_as_bytes(uploaded_image).await?;
    s3_client
        .put_user_profile_avatar(&user_id, content)
        .await
        .map_err(e500)?;
    UserProfile::set_avatar(&user_id, &s3_client.endpoint, &pool).await?;
